""" Training routine for 3D parking location detection with SUN RGB-D.

Sample usage:
python train.py --dataset sunrgbd --log_dir log_sunrgbd
Training command used by Shivam:
python train.py --log_dir log_parknet --dump_dir dump_parknet --overwrite
To use Tensorboard:
At server:
...
parser.add_argument('--model', default='parknet', help='Model file name [default: parknet]')
parser.add_argument('--dataset', default='sunrgbd', help='Dataset name. sunrgbd or scannet. [default: sunrgbd]')
parser.add_argument('--checkpoint_path', default=None, help='Model checkpoint path [default: None]')
parser.add_argument('--log_dir', default='log', help='Dump dir to save model checkpoint [default: log]')
parser.add_argument('--dump_dir', default=None, help='Dump dir to save sample outputs [default: None]')
parser.add_argument('--num_point', type=int, default=2000, help='[Removed from Parknet] Point Number [default: 20000]')
parser.add_argument('--num_target', type=int, default=128, help='Proposal number [default: 64]')
...
if FLAGS.dataset == 'sunrgbd':
    ...
    # Define dataset
    TRAIN_DATASET = ParknetDetectionVotesDataset(split_set='train', num_points=NUM_POINT, use_height=(not FLAGS.no_height), use_color=FLAGS.use_color, augment=True)
    TEST_DATASET = ParknetDetectionVotesDataset(split_set='val', num_points=NUM_POINT, augment=False, use_color=FLAGS.use_color, use_height=(not FLAGS.no_height))
else:
    print('Unknown dataset %s. Exiting...'%(FLAGS.dataset))
    exit(-1)

print("Training Samples = {}, Testing Samples = {}".format(len(TRAIN_DATASET), len(TEST_DATASET)))
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=BATCH_SIZE,
     shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn)
TEST_DATALOADER = DataLoader(TEST_DATASET, batch_size=BATCH_SIZE,
     shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn)
print("Training Batches = {}, Testing Batches = {}".format(len(TRAIN_DATALOADER), len(TEST_DATALOADER)))
...
print("Number of GPU's Available = {}".format(torch.cuda.device_count()))
...
CONFIG_DICT = { 'remove_colliding_placements_scene': False, 'outlier_removal': False, 'remove_colliding_placements_object': False,
...
def evaluate_one_epoch():
    ...
    # Evaluate average precision NOTE: I have not USED Scene data here!
...
if __name__=='__main__':
    ...
    print('Dump_dir = {}'.format(DUMP_DIR))
    print('Everything set fine')
    train(start_epoch)