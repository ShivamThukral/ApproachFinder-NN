...
# def compute_heading_weight_loss(end_points, config):
#
#     spot_assignment = end_points['spot_assignment']
#     pred_park_mask = end_points['pred_park_mask'].float()
#
#     # Compute heading loss
#     heading_class_label = torch.gather(end_points['heading_class_label'], 1,
#                                        spot_assignment)  # select (B,K) from (B,K2)
#     criterion_heading_class = nn.CrossEntropyLoss(reduction='none')
#     heading_class_loss = criterion_heading_class(end_points['heading_scores'].transpose(2, 1),
#                                                  heading_class_label)  # (B,K)
#     heading_class_loss = torch.sum(heading_class_loss * pred_park_mask) / (torch.sum(pred_park_mask) + 1e-6)
#
#
#     # Compute weight loss
#     weight_class_label = torch.gather(end_points['weight_class_label'],1, spot_assignment)
#     criterion_weight_class = nn.CrossEntropyLoss(reduction='none')
#     weight_class_loss = criterion_weight_class(end_points['weight_scores'].transpose(2, 1), weight_class_label)  # (B,K)
#     weight_class_loss = torch.sum(weight_class_loss * pred_park_mask) / (torch.sum(pred_park_mask) + 1e-6)
#
#     return heading_class_loss, weight_class_loss
...
def get_loss(end_points, config):
    ...
    # heading_class_loss, weight_class_loss = compute_heading_weight_loss(end_points,config)
    # end_points['heading_class_loss'] = heading_class_loss
    # end_points['weight_class_loss'] = weight_class_loss
