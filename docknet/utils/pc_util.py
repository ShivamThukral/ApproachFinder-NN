...
def write_parking_ply(spots, filename, text=True):
    """ write the parking spots into a file """
    fout = open(filename, 'w')
    for spot in spots:
        fout.write("{}\n".format(spot))
    fout.close()

    # spot_vertex = [(spots[i, 0], spots[i, 1], spots[i, 2]) for i in range(spots.shape[0])]
    # spot_heading = [[spots[i, j] for j in range(spots.shape[1]-11)] for i in range(spots.shape[0])]
    # print(spot_heading)
    # vertex = np.array(spot_heading, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4'),( 'h1', 'f4')])
    # #el = PlyElement.describe(vertex, 'my_vertex', comments=['vertices'])
    # # PlyData([el], text=text).write(filename)
    # print('yes')
