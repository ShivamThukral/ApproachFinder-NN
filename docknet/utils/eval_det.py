...
def eval_det_cls(pred, gt, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou):
    ...
    print('class recs = {}'.format(len(class_recs)))
    # construct dets
    image_ids = []
    confidence = []
    BB = []
...
def my_eval_det_multiprocessing(pred_all, gt_all, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou):
    """ Generic functions to compute precision/recall for object detection
        for multiple classes.
        Input:
            pred_all: map of {img_id: [(score, center, weight)]}
            gt_all: map of {img_id: [(center, weight)]}
            ovthresh: scalar, iou threshold
            use_07_metric: bool, if true use VOC07 11 point method
        Output:
            rec: {classname: rec}
    ...
    # print("pred  len = {}".format(len(pred_all)))
    # print("gt  len = {}".format(len(gt_all)))
    for img_id in pred_all.keys():
        for score, center, weight, bbox in pred_all[img_id]:
            if classname not in pred: pred[classname] = {}
            if img_id not in pred[classname]:
                pred[classname][img_id] = []
            if classname not in gt: gt[classname] = {}
            ...
            pred[classname][img_id].append((bbox, score))
    for img_id in gt_all.keys():
        for center, weight, bbox in gt_all[img_id]:
            if classname not in gt: gt[classname] = {}
            if img_id not in gt[classname]:
                gt[classname][img_id] = []
            gt[classname][img_id].append(bbox)
    # print(len(pred[classname]))
    # print(len(gt[classname]))
    rec = {}
    prec = {}
    ap = {}
    p = Pool(processes=1)  # one class so one thread
