...
DEFAULT_TYPE_WHITELIST = ['table', 'chair', 'toilet']
...
class sunrgbd_object(object):
    ...
    def __init__(self, root_dir, split='training', use_v1=False):
        ...
        self.depth_image_dir = os.path.join(self.split_dir, 'depth_image')
    ...
    def get_depth_image(self, idx):
        depth_image_filename = os.path.join(self.depth_image_dir, '%06d.png' % (idx))
        return sunrgbd_utils.load_depth_image(depth_image_filename)


def data_viz(data_dir, dump_dir=os.path.join(BASE_DIR, 'data_viz_dump')):
    ''' Examine and visualize SUN RGB-D data. '''
    sunrgbd = sunrgbd_object(data_dir)  # S : read the dataset object
    ...
    for idx in range(len(sunrgbd)):
        ...
        pc = sunrgbd.get_depth(data_idx)  # Nx6 = XYZ + RGB
        print('Point cloud shape:', pc.shape)

        # Project points to image
        calib = sunrgbd.get_calibration(data_idx)
        uv, d = calib.project_upright_depth_to_image(pc[:, 0:3])  # S : UV = [x,y] and d = value
        print('Point UV:', uv)
        print('Point depth:', d)
        ...
        img = sunrgbd.get_image(data_idx)
        Image.fromarray(img).save(os.path.join(dump_dir, 'img.jpg'))
        d_img = sunrgbd.get_depth_image(data_idx)
        d_img.save(os.path.join(dump_dir, 'img_depth_gt.png'))
        #Image.fromarray(d_img).save(os.path.join(dump_dir, 'img_depth_gt.png'))
        ...
        img = sunrgbd.get_image(data_idx)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # S : image is store in BGR format be careful
        ...
        for num_point in [10000, 20000, 40000, 80000]:
            ...
            pc_util.write_ply_rgb(sampled_pcrgb[:, 0:3],
                                  ...
                                  os.path.join(dump_dir, 'pcrgb_%dk.obj' % (
                                              num_point // 1000)))  # dump the object file as XYZ + RGB (0-256)
...
def extract_sunrgbd_data(idx_filename, split, output_folder, num_point=20000,
    ...
    Args:
        ...
        skip_empty_scene: if True, skip scenes that contain no object (no object in whitelist)
    ...
    for data_idx in data_idx_list:
        ...
        pc_upright_depth = dataset.get_depth(data_idx)  # S: get the depth mat
        pc_upright_depth_subsampled = pc_util.random_sampling(pc_upright_depth, num_point)  # randomly sample points
        ...
        # code added by ST to save jpg image, png depth image and calib matrix
        calib = dataset.get_calibration(data_idx)
        calib_params = np.vstack((calib.Rtilt, calib.K))
        np.save(os.path.join(output_folder, '%06d_calib.npy' % (data_idx)), calib_params)

        img = dataset.get_image(data_idx)
        Image.fromarray(img).save(os.path.join(output_folder, '%06d_img.jpg' % (data_idx)))

        depth_img = dataset.get_depth_image(data_idx)
        depth_img.save(os.path.join(output_folder, '%06d_depth_img.png' % (data_idx)))
        #Image.fromarray(depth_img).save(os.path.join(output_folder, '%06d_depth_img.png' % (data_idx)))
        # Code ended

        if save_votes:
            N = pc_upright_depth_subsampled.shape[0]  # S: find the shape of the subsampled point cloud
            ...
            for obj in objects:
                ...
                try:
                    ...
                    box3d_pts_3d = sunrgbd_utils.my_compute_box_3d(obj.centroid,
                                                                   ...
                                                                   obj.heading_angle)  # ============>  S: convert the BB params to 8x3 bounding box values
                    pc_in_box3d, inds = sunrgbd_utils.extract_pc_in_box3d( \
                        pc_upright_depth_subsampled, box3d_pts_3d)  # S: xyz + index from N
                    # Assign first dimension to indicate it is in an object box
                    point_votes[inds, 0] = 1
                    # Add the votes (all 0 if the point is not in any object's OBB)
                    votes = np.expand_dims(obj.centroid, 0) - pc_in_box3d[:,
                                                              0:3]  # S: expand will create row vector which we substract abs distance
...
if __name__ == '__main__':
    ...
    #ST: this code fragment is not used so unchanged.
    if args.gen_v1_data:
        ...
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
                             ...
                             save_votes=True, num_point=50000, use_v1=True, skip_empty_scene=False)
    #ST: used to generate the parknet dataset
    if args.gen_v2_data:
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
                             split='training',
                             output_folder=os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_80k_v2_train'),
                             save_votes=False, num_point=80000, use_v1=False, skip_empty_scene=True)
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
                             split='training',
                             output_folder=os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_80k_v2_val'),
                             save_votes=False, num_point=80000, use_v1=False, skip_empty_scene=True)
